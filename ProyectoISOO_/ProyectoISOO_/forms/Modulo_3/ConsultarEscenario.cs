...
{
    public partial class ConsultarEscenario : Form
    {
        public ConsultarEscenario()
        {
            InitializeComponent();
        }
        void CargaInicial()
        {
            try
            {
                string procedure = "PROC_CONSULTAR_ESCENARIO";
                clases.Modulo_3.Consulta(dgapp, procedure, "ninguno");
                dgapp.Columns[0].HeaderText = "Código";
                dgapp.Columns[1].HeaderText = "Escenario";
                dgapp.Columns[2].HeaderText = "Tema";
        ...
        {
            forms.Modulo_3.GestionEscenario tema = new forms.Modulo_3.GestionEscenario();
            tema.opcion = 0;
            tema.ShowDialog();
            CargaInicial();
        }
        ...
        private void btneliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                if (clases.Modulo_3.Eliminar(Int32.Parse(dgapp.Rows[dgapp.CurrentRow.Index].Cells[0].Value.ToString()),"PROC_ELIMINAR_ESCENARIO") > 0)
                {
